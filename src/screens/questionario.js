...
class ScreenQuestionario extends Component {
  ...
  prepareQuestions() {
    ...
    const PosterId = data.numero;
    this.setState({ PosterTitulo: data.titulo });
    this.setState({ PosterId: data.numero });
    ...
    let dados = [
      ...
      },
      {
        Id: 4,
        Texto:
          'Análise de dados e resultados: articulação teórica e metodológica da interpretação.Conclusões: coerência e alcance.',
        Nota: 0,
        PosterId: PosterId,
        Prontuario: prontuario
      },
  ...
  confirmSave() {
    ...
    Alert.alert(
      'Confirmar Avaliação',
      `Deseja confirmar sua avaliação para o poster '${PosterTitulo}'?`,
      [
        {
          text: 'Cancelar',
          ...
          renderItem={({ item }) => (
            <View style={styles.container}>
              ...
              <Slider
                ...
                value={item.Nota}
                onValueChange={value =>
                  this.handleSliderValueChange(value, item.Id)
                }
              />
